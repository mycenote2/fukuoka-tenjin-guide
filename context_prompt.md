...
19. (2026-07-15) **GitHub 저장소 생성 + 코드 푸시**: https://github.com/mycenote2/fukuoka-tenjin-guide (public). gh CLI가 brew로 설치 불가(Xcode 낡음)여서 공식 gh v2.96.0 바이너리를 scratchpad에 받아 mycenote2로 로그인 후 `gh repo create --push`. Vercel 프로젝트 rootDirectory를 API로 'site'로 설정(Git 배포 시 site/ 기준). ※ 챗(claude.ai) 쪽 GitHub 커넥터는 이 코딩 세션 도구목록에 안 잡혀서 사용 불가.
20. (2026-07-15) **GitHub↔Vercel 자동배포 연결 완료·검증**: 사용자가 Vercel 대시보드에서 GitHub App 승인 후 저장소 연결(프로덕션 브랜치 main). README.md 추가 커밋을 push하니 source=git 배포가 자동 생성·READY, 프로덕션이 그 커밋(fb2fcbf)을 가리킴. 메인 200·게시판 API 200(서버리스+Redis 정상, rootDirectory=site 검증)·ads.txt·AdSense 스크립트 모두 라이브 확인. **이제 `git push origin main`만 하면 자동 재배포됨.** ※ CLI 수동배포가 필요하면 repo 루트가 아니라 여전히 site/에서 하거나, 그냥 git push 사용 권장.
